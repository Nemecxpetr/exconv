...
def test_impulse_self_convolution_same_modes():
    ...
    # same-first
    out_first = auto_convolve(
        a, mode="same-first", circular=False, normalize=None, order=2
    )
    expected_first = full[: x.size]
    assert out_first.samples.shape == x.shape
    np.testing.assert_allclose(
        out_first.samples, expected_first, rtol=1e-10, atol=1e-10
    )

    # same-center
    out_center = auto_convolve(
        a, mode="same-center", circular=False, normalize=None, order=2
    )
    L = full.size
    N = x.size
    start = (L - N) // 2
    ...
    np.testing.assert_allclose(
        out_center.samples, expected_center, rtol=1e-10, atol=1e-10
    )
...
def test_stereo_per_channel_convolution():
    ...
    np.testing.assert_allclose(
        out.samples[:, 1], expected_right, rtol=1e-10, atol=1e-10
    )
...
def test_neutral_spectral_processing_matches_plain_convolution():
    sr = 8000
    x = np.random.default_rng(10).standard_normal(64)
    h = np.random.default_rng(11).standard_normal(16)
    ...
    shaped = pair_convolve(
        Audio(samples=x, sr=sr),
        Audio(samples=h, sr=sr),
        mode="full",
        normalize=None,
        spectral_processing=AudioSpectralProcessing(),
    )

    np.testing.assert_allclose(shaped.samples, plain.samples, rtol=1e-10, atol=1e-10)


def test_spectral_high_gain_enhances_high_frequency_component():
    sr = 8000
    n = 256
    t = np.arange(n) / sr
    ...
    raw_mag = np.abs(np.fft.rfft(raw.samples))[bin_1k]
    bright_mag = np.abs(np.fft.rfft(bright.samples))[bin_1k]

    assert bright_mag > raw_mag * 1.9


def test_low_preserve_keeps_low_bin_closer_to_reference():
    sr = 8000
    n = 256
    t = np.arange(n) / sr
    ...
    freqs = np.fft.rfftfreq(n, 1.0 / sr)
    low_bin = int(np.argmin(np.abs(freqs - 125.0)))
    x_mag = np.abs(np.fft.rfft(x))[low_bin]
    raw_mag = np.abs(np.fft.rfft(raw.samples))[low_bin]
    preserved_mag = np.abs(np.fft.rfft(preserved.samples))[low_bin]

    assert abs(preserved_mag - x_mag) < abs(raw_mag - x_mag)
