...
exconv.xmodal    # cross-modal tools (sound→image), via submodules
exconv.__version__
```
...
| `real_input`   | bool                      | If `True`, uses `rfftn` (real→complex).        |
| `real_output`  | bool                      | If `True`, uses `irfftn` (complex→real).       |
...
- `radial_grid_2d(H, W, norm="unit") -> ρ`  
...
- `normalize`:
  - `"clip"`: clip to input’s numeric range.
...
| `normalize`| If `True`, normalizes the final output: luma→global [0,1], color→clipped to [0,1]. |
...
Provides a CLI façade over `spectral_sculpt` for quick experiments.
